...
```typescript
let calls = 0;
const modelCall = async () => { calls += 1; return 'reply'; };
...
**CLI smoke test:**
```typescript
const result = spawnSync('npx', ['tsx', 'scripts/flue-eval.ts', 'run', configPath, '--json'], { cwd, env, timeout });
